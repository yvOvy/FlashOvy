...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( function() {
	...
	var _DEG2RAD = Math.PI / 180,
		...
		_sortBezierSegmentLength = function(a, b) {
			return b.length - a.length;
		},
		/*
		_PI = Math.PI,
		_2PI = _PI * 2,
		_acuteAngle = function(angle) { //accepts a radian angle and spits back the equivalent acute angle, so 270 degrees would become -90.
		    angle = angle % _2PI;
		    return (angle === angle % _PI) ? angle : (angle < 0) ? angle + _2PI : angle - _2PI;
		 },
		...
		_getClosestShapeIndex = function(bezier, angle) { //finds the index in a closed cubic bezier array that's closest to the angle provided (angle measured from the center or average x/y).
			var l = bezier.length,
				averages = _getAverageXY(bezier),
				x = averages[0],
				y = averages[1],
				closestDistance = Math.abs(_acuteAngle(Math.atan2(bezier[1] - y, bezier[0] - x) - angle)),
				closestIndex = 0,
				start = 6 * (((l-2) / 4 / 6) | 0), //exclude the first and final 25% of the anchors because we're really just trying to find if it's likely to invert. In other words, we want to shy away from using a point that's really close to the start (in that case just use the start because most of the time that's what users want).
				end = 6 * ((3 * (l-2) / 4 / 6) | 0),
				d, i;
			//console.log("start angle: ", _acuteAngle(Math.atan2(bezier[1] - y, bezier[0] - x)) * _RAD2DEG, " trying to get close to ", angle * _RAD2DEG);
			for (i = start; i < end; i += 6) {
				d = Math.abs(_acuteAngle(Math.atan2(bezier[i+1] - y, bezier[i] - x) - angle));
				//console.log(_acuteAngle(Math.atan2(bezier[i+1] - y, bezier[i] - x)) * _RAD2DEG, ":", i);
				if (d < closestDistance) {
					closestDistance = d;
					closestIndex = i;
				}
			}
			//console.log("closest shape index angle: ", _acuteAngle(Math.atan2(bezier[closestIndex+1] - y, bezier[closestIndex] - x) ) * _RAD2DEG, "(index: ", closestIndex, ")");
			return closestIndex / 6;
		},
		*/
		...
		_getTotalMovement = function(sb, eb, shapeIndex) {
			var l = sb.length,
				d = 0,
				wrap = l - 2,
				index, i;
			...
			for (i = 0; i < l; i += 6) {
				index = (i + shapeIndex) % wrap;
				d += Math.abs(sb[index] - eb[i]) + Math.abs(sb[index+1] - eb[i+1]);
			}
			return d;
		},
		_getClosestShapeIndex = function(sb, eb, checkReverse) { //finds the index in a closed cubic bezier array that's closest to the angle provided (angle measured from the center or average x/y).
			var l = sb.length,
				min = _getTotalMovement(sb, eb, 0),
				minIndex = 0,
				copy, d, i;
			for (i = 6; i < l; i += 6) {
				d = _getTotalMovement(sb, eb, i / 6);
				if (d < min) {
					min = d;
					minIndex = i;
				}
			...
			if (checkReverse) {
				...
				_reverseBezier(copy);
				for (i = 6; i < l; i += 6) {
					d = _getTotalMovement(copy, eb, i / 6);
					if (d < min) {
						min = d;
						minIndex = -i;
					}
		...
		_getClosestAnchor = function(bezier, x, y) { //finds the index of the anchor that's closest to the provided x/y coordinate (the index returned is the x-coordinate in that bezier, so the y would be one greater)
			var l = bezier.length,
				...
				d, i;
			for (i = 0; i < l; i += 6) {
				d = Math.abs(bezier[i] - x) + Math.abs(bezier[i+1] - y);
				if (d < closestDistance) {
					closestDistance = d;
					closestIndex = i;
				}
		...
		_equalizeSegmentQuantity = function(start, end, shapeIndex) { //returns an array of shape indexes, 1 for each segment.
			...
			if (dif) {
				...
				start.sort(_sortBezierSegmentLength);
				end.sort(_sortBezierSegmentLength);
			...
			for (i = 0; i < start.length; i++) {
				...
				if (shapeIndex) {
					...
					if (Math.abs(sb[0] - sb[sb.length - 2]) < 0.01 && Math.abs(sb[1] - sb[sb.length - 1]) < 0.01) {
						if (shapeIndex === "auto") {
		...
		_pathFilter = function(a, shapeIndex) {
			var start = _pathDataToBezier(a[0]),
				end = _pathDataToBezier(a[1]);
			_equalizeSegmentQuantity(start, end, (shapeIndex || shapeIndex === 0) ? shapeIndex : "auto");
			a[0] = _bezierToPathData(start);
			a[1] = _bezierToPathData(end);
		},
		_buildPathFilter = function(shapeIndex) {
			return (shapeIndex || shapeIndex === 0) ? function(a) {
				_pathFilter(a, shapeIndex);
			} : _pathFilter;
		},
		...
		_convertToPath = function(e, swap) {
			...
			if (type === "rect") {
				...
				if (r || ry) { //if there are rounded corners, render cubic beziers
					...
					y5 = y4 + ry * circ;
					y6 = y4 + ry;
					data = "M" + x6 + "," + y3 + " V" + y4 + " C" + [x6, y5, x5, y6, x4, y6, x4 - (x4 - x3) / 3, y6, x3 + (x4 - x3) / 3, y6, x3, y6, x2, y6, x, y5, x, y4, x, y4 - (y4 - y3) / 3, x, y3 + (y4 - y3) / 3, x, y3, x, y2, x2, y, x3, y, x3 + (x4 - x3) / 3, y, x4 - (x4 - x3) / 3, y, x4, y, x5, y, x6, y2, x6, y3].join(",");
				} else {
				...
				rcirc = r * circ;
				data = "M" + (x+r) + "," + y + " C" + [x+r, y + rycirc, x + rcirc, y + ry, x, y + ry, x - rcirc, y + ry, x - r, y + rycirc, x - r, y, x - r, y - rycirc, x - rcirc, y - ry, x, y - ry, x + rcirc, y - ry, x + r, y - rycirc, x + r, y].join(",");
			} else if (type === "line") {
		...
		MorphSVGPlugin = _gsScope._gsDefine.plugin({
			...
			init: function(target, value, tween) {
				...
				this._target = target;
				pt = this._addTween(target, "setAttribute", target.getAttribute(p) + "", shape + "", "morphSVG", false, p, (p === "d") ? _buildPathFilter(value.shapeIndex) : _buildPointsFilter(value.shapeIndex));
				if (pt) {
	...
	MorphSVGPlugin.convertToPath = function(targets, swap) {
		...
		var a = (!targets || targets.length === 0) ? [] : (targets.length && targets[0] && targets[0].nodeType) ? targets : [targets],
			i = a.length;
		while (--i > -1) {
