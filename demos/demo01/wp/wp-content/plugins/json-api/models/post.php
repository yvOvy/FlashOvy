      ...
      foreach ($wp_custom_fields as $key => $value) {
        if ($json_api->query->custom_fields) {
          if (in_array($key, $keys)) {
			echo "setukje klucz ".$key;
			$this->custom_fields->$key = $wp_custom_fields[$key];
		  }
		} else if (substr($key, 0, 1) != '_') {
			if(!isset($this->custom_fields->$key)) $this->custom_fields->$key = $wp_custom_fields[$key];
        }
      }*/
	} else {
  ...
  function json_api_add_acf(&$post) 
	{
		...
		// $post->custom_fields->buttons = get_field( 'buttons', "option" );
